...
resource "aws_dynamodb_resource_policy" "this" {
  ...
  policy       = replace(replace(var.resource_policy, "__DYNAMODB_TABLE_ARN__", local.dynamodb_table_arn),"__AWS_ACCOUNT_NUMBER__",data.aws_caller_identity.current.account_id)
}
...
resource "aws_ssm_parameter" "dynamodb_table" {
  ...
  type  = "String"
  value = aws_dynamodb_table.this[count.index].name
}